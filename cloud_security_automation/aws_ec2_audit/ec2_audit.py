...
def run_ec2_audit(ec2_client=None, iam_client=None) -> List[AuditFinding]:
    ...
    if not instances:
        logger.warning(
            ...
            "EC2 instance level checks were skipped."
        )
    ...
    if not any(f.status == "FAIL" for f in findings):
        ...
        if failed:
            logger.info(
                "EC2 audit completed with %d failed checks, see results for details",
                failed,
            )

        else:
            logger.info("EC2 audit completed successfully with no findings")
